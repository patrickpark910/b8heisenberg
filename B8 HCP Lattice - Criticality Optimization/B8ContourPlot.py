...
# ---------------------------
GRID_N = 500   # grid resolution per axis (lower than 1000 for speed; increase if needed)
SIGMA  = 3       # Gaussian blur strength in grid cells; try 1.5–3 for gentle smoothing
LEVELS = np.linspace(0.90, 1.02, num=13)
FONTSIZE = 15

# Conversion constants
RHO_HW_G_CM3 = 1.106104        # heavy water density in g/cm^3
KG_PER_LITER = RHO_HW_G_CM3    # since 1 L = 1000 cm^3 and 1 kg = 1000 g
...
def liters_to_kg(L):
    ...
    return L * KG_PER_LITER

def kg_to_liters(kg):
    """Convert kilograms of D2O (ρ = 1.1 g/cm^3) to liters."""
    return kg / KG_PER_LITER

def cubes_to_kg(nc):
    """Convert number of uranium cubes to kilograms (125 cm^3 each, ρ = 18.5349 g/cm^3)."""
    return nc * CUBE_MASS_KG

def kg_to_cubes(kg):
    """Convert kilograms of uranium cubes to number of cubes."""
    return kg / CUBE_MASS_KG
...
xi = np.linspace(c.min(), c.max(), GRID_N)
yi = np.linspace(m.min(), m.max(), GRID_N)
X, Y = np.meshgrid(xi, yi)

# 1) Use a milder interpolant (linear) to avoid cubic ringing
Z_lin = griddata((c, m), k, (X, Y), method='linear')

# 2) Smooth the interpolated field with a NaN-aware Gaussian blur
Z = gaussian_filter_nan(Z_lin, sigma=SIGMA)
...
cbar = fig.colorbar(contour_filled, ax=ax, pad=0.12, fraction=0.035)  # <-- pad bumped up
...
# SECONDARY AXES
# ---------------------------
# Right Y: liters -> kg (ρ = 1.1 g/cm^3)
...
# Top X: number of cubes -> kg (125 cm^3 each, ρ = 18.5349 g/cm^3)
...
RATIO_M_TO_U = 18.6     # V_D2O / V_U
CUBE_VOL_L   = CUBE_VOL_CM3 / 1000.0  # 0.125 L per cube

# Dotted line where V_D2O / V_U = 18.6  →  m = 18.6 * 0.125 * c = 2.325 * c
x_line = np.linspace(c.min(), c.max(), 500)
y_line = RATIO_M_TO_U * CUBE_VOL_L * x_line
ax.plot(x_line, y_line, linestyle=':', linewidth=2, color='k')

# Optional label in axes coordinates (comment out if you don’t want it)
...
# ---------------------------
# NEW: EXTRACT AND LABEL K-EFF = 1.00 CONTOUR
# ---------------------------
# Extract the k-eff = 1.00 contour
k_1_contour = ax.contour(X, Y, Z, levels=[1.00], colors='red', linewidths=2, alpha=0.8)

# Get the vertices of the k-eff = 1.00 contour line
if len(k_1_contour.get_paths()) > 0 and len(k_1_contour.get_paths()[0]) > 0:
