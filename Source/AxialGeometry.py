...
class AxialGeometry(MCNP_File):

    def process_axial_keff(self):
        ...
        if self.run_type in ['axial_z']:
            ...
            self.keff_filename = f'{self.base_filename}_{self.run_type}_{self.n_cubes_chain_a}_{self.n_cubes_chain_b}_keff.csv'
            self.keff_filepath = f"{self.results_folder}/{self.keff_filename}"
